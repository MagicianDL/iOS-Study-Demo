...
- (void)viewDidLoad {
    ...
    /*
     NSNumberFormatterNoStyle = kCFNumberFormatterNoStyle, //无格式 四舍五入的整数
     
     NSNumberFormatterDecimalStyle = kCFNumberFormatterDecimalStyle, //小数型
     
     NSNumberFormatterCurrencyStyle = kCFNumberFormatterCurrencyStyle, //货币型 本地化
     ...
     NSNumberFormatterSpellOutStyle = kCFNumberFormatterSpellOutStyle  //全拼 本地化
     
     NSNumberFormatterOrdinalStyle = kCFNumberFormatterOrdinalStyle, //序数 本地化
     
     NSNumberFormatterCurrencyISOCodeStyle = kCFNumberFormatterCurrencyISOCodeStyle, //货币， ISO 本地化
     
     NSNumberFormatterCurrencyPluralStyle = kCFNumberFormatterCurrencyPluralStyle, //货币，本地化
     
     NSNumberFormatterCurrencyAccountingStyle = kCFNumberFormatterCurrencyAccountingStyle, // 会计技术 本地化
     */
    
    
    NSNumber *doubleNumber                     = [NSNumber numberWithDouble:123456.8396];
    // 四舍五入的整数
    NSString *numberNoStyleStr                 = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterNoStyle];

    // 小数形式
    NSString *numberDecimalStyleStr            = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterDecimalStyle];

    // 货币形式 -- 本地化
    NSString *numberCurrencyStyleStr           = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterCurrencyStyle];

    // 百分数形式
    NSString *numberPercentStyleStr            = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterPercentStyle];

    // 科学计数
    NSString *numberScientificStyleStr         = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterScientificStyle];

    // 朗读形式 -- 本地化
    NSString *numberSpellOutStyleStr           = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterSpellOutStyle];

    // 序数形式 -- 本地化
    NSString *numberOrdinalStyleStr            = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterOrdinalStyle];

    // 货币形式 ISO -- 本地化
    NSString *numberCurrencyISOStyleStr        = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterCurrencyISOCodeStyle];

    // 货币形式 -- 本地化
    NSString *numberCurrencyPluralStyleStr     = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterCurrencyPluralStyle];

    // 会计计数 -- 本地化
    NSString *numberCurrencyAccountingStyleStr = [NSNumberFormatter localizedStringFromNumber:doubleNumber numberStyle:NSNumberFormatterCurrencyAccountingStyle];
    ...
    NSLog(@"Spell Out Style                    = %@",numberSpellOutStyleStr);// Spell Out Style           = one million two hundred thirty-four thousand five hundred sixty-seven point eight three six nine
