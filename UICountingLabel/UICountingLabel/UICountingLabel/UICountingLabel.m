...
-(void)countFrom:(CGFloat)value to:(CGFloat)endValue {
    ...
    [self countFrom:value to:endValue duration:self.animationDuration];
}
...
{
    [self countFrom:[self currentValue] to:endValue duration:duration];
}
...
- (void)setTextValue:(CGFloat)value
{
    ...
    if (self.currentValueBlock) {
        self.currentValueBlock(self, value);
    }
}
